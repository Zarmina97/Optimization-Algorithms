...
Data extraction
'''
data = pd.read_csv('csv folder/Salary_Data.csv')
...
'''
Correlation plot to show how much the attribute correlates
'''
...
plt.savefig('images_Salary/Correlation matrix_Salary.png')
plt.show()
plt.close()
...
model_gd = MySGD(learning_rate = 0.01)
...
model_sgd = MySGD(learning_rate = 0.01)
history_sgd, _ = model_sgd.fit(X_train, y_train, batch_size = 8, epochs = 100)
...
model_adam = MyAdam(learning_rate = 0.1)
history_adam, _ = model_adam.fit(X_train, y_train, batch_size = 8, epochs = 100)
...
ax.set_xlim(0,150)
ax.set_ylim(-0.2, 1.5)
ax.set_title('Animation of losses')
...
plt.savefig('images_Salary/Losses_Salary.png')
plt.show()
...
plt.savefig('images_Salary/Losses with x-axes shared_Salary.png')
plt.show()
...
fig.tight_layout()
plt.savefig('images_Salary/Timing gd vs sgd_Salary')
plt.show()
...
for lr in learning_rate:
    for bs in batchs:
        for e in epsilon:
            ...
            plt.ylabel('Cost')
    plt.legend()
    plt.savefig('images_Salary/adam parameters with lr={}_Salary.png'.format(lr))
    plt.show()
...
plt.legend()
fig.tight_layout()
plt.savefig('images_Salary/MSE_Salary.png')
plt.show()
...
plt.savefig('images_Salary/Regression curve_Salary.png')
plt.show()
...
history_gd, n_iter_gd = model_gd.fit(X_train, y_train, batch_size=1,epochs=100, early_stopping=True)
print("GD n_iter: " + str(n_iter_gd))

print("==============SGD==============")
model_sgd = MySGD(learning_rate = 0.001)
history_sgd, n_iter_sgd = model_sgd.fit(X_train, y_train, batch_size = 32, epochs = 100, early_stopping=True)
print("SGD n_iter: " + str(n_iter_sgd))

print("=============ADAM=============")
model_adam = MyAdam(learning_rate = 0.01)
history_adam, n_iter_adam = model_adam.fit(X_train, y_train, batch_size = 32, epochs = 100, early_stopping=True)
